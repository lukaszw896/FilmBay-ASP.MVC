...
namespace FilmBayMVC
{
    public static class DBAccess
    {
        ...
        public static void UpdateComment(comment_table t)
        {
            using (MyLINQDataContext conn = new MyLINQDataContext())
            {
                comment_table table = conn.comment_tables.AsParallel().Where(s => s.id_film == t.id_film && s.id == t.id).FirstOrDefault();
        ...
        public static void UpdateVote(vote_table t)
        {
            using (MyLINQDataContext conn = new MyLINQDataContext())
            {
                vote_table table = conn.vote_tables.AsParallel().Where(s => s.id_film == t.id_film && s.id == t.id).FirstOrDefault();
        ...
        public async static Task<List<comment_table>> GetComments(int filmid)
        {
            return await Task.Run(() =>
            {
                ...
                Comments = (from a in con.comment_tables
                            join u in con.AspNetUsers on a.id equals u.Id
                            join f in con.film_tables on a.id_film equals f.id_film
                            where f.id_film == filmid
                            select a).ToList();
                con.Dispose();
        ...
        public async static Task<bool> BuyFilm(int filmid, string userid)
        {
            return await Task.Run(() =>
            {
                ...
                AspNetUser x = new AspNetUser();
                x = (con.AspNetUsers.AsParallel().Where(s => s.Id == userid).FirstOrDefault());
                bought_films_table bft = new bought_films_table();
                bool Alreadybought = (con.bought_films_tables.AsParallel().Where(s => s.id_film == filmid && s.id == userid).Count()) > 0;
        ...
        public async static Task<int> VoteForFilm(int filmid, string userid, int vote)
        {
            // 6- admin
            return await Task.Run(() =>
            {
                ...
                bool Alreadyvoted = (con.vote_tables.AsParallel().Where(s => s.id_film == filmid && s.id == userid).Count()) > 0;
                // bool isAdmin = (x.is_admin) == 1;
                ...
                {
                    bft = con.vote_tables.AsParallel().Where(s => s.id_film == filmid && s.id == userid).FirstOrDefault();
                    int old = bft.vote;
        ...
        public async static Task<List<film_table>> GetBoughtFilms(string id)
        {
            return await Task.Run(() =>
            {
                MyLINQDataContext con = new MyLINQDataContext();
                List<film_table> FilmTables = (from u in con.AspNetUsers
                                               join bf in con.bought_films_tables on u.Id equals bf.id
